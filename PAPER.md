...
Agentic commerce — the paradigm in which autonomous AI agents discover, negotiate, and complete purchases on behalf of human principals — has moved from speculative concept to deployed reality in under eighteen months. As of early April 2026, ChatGPT's shopping experience (pivoting from Instant Checkout to retailer-owned apps via Shopify Agentic Storefronts across 5.6 million merchants) reaches 900 million weekly users, Google's Universal Commerce Protocol (UCP) has the backing of Shopify, Walmart, Target, and PayPal, Stripe's Machine Payments Protocol (MPP) is live across more than 100 services, Santander and Mastercard have completed Europe's first live end-to-end payment executed by an AI agent within a regulated banking framework, and Alibaba has deployed autonomous "digital employees" for millions of Taobao and Tmall merchants — marking the first large-scale agentic commerce rollout in China. This paper surveys the technical infrastructure, key players, economic models, trust frameworks, regulatory landscape, and open research questions defining this nascent but rapidly consolidating field. On April 2, 2026, the x402 protocol was donated to the Linux Foundation with 20+ founding members — including AWS, American Express, Google, Mastercard, Microsoft, Shopify, and Visa — marking a significant governance milestone for open agent payment standards. At Stripe Sessions 2026 (April 29), Stripe unveiled 288 new products and features — headlined by **Link's wallet for agents** (consumer-controlled agent spending via one-time-use cards) and a **Google partnership** enabling merchants to sell inside AI Mode and the Gemini app — with CEO Patrick Collison declaring that "in the not-too-distant future agents will account for most transactions online." On May 1, 2026, **Amazon, Meta, Microsoft, Salesforce, and Stripe officially joined the UCP Tech Council**, doubling its membership to 10 organizations and transforming UCP from a Google-Shopify initiative into a genuine multi-stakeholder industry standard. Visa expanded its Agentic Ready program globally to Asia Pacific (enrolling **50+ issuer partners across 10 markets**) and Latin America, Alipay launched an AI payment processing product for agent commerce in China, Oobit launched Tether-backed **Visa Agent Cards** for AI agents to spend USDT, and new entrants including Alchemy (AgentPay), OKX (Agent Payments Protocol), and Clink (fiat Agentic Payment Skill) further densified the payment infrastructure layer. McKinsey projects agentic commerce will orchestrate $3–5 trillion in global retail spend by 2030; Juniper Research projects $1.5 trillion globally (identifying trust as the primary adoption barrier); Bain & Co. estimates $300–500 billion in the U.S. alone (15–25% of e-commerce), while Morgan Stanley projects $190–385 billion (10–20%). Amazon's Rufus AI assistant has reached 300 million users and driven $12 billion in incremental annualized sales, while Mirakl and J.P. Morgan Payments have partnered to bring agentic checkout to enterprise marketplaces. Experian launched Agent Trust™ — a first-of-its-kind "Know Your Agent" identity framework developed with Visa, Cloudflare, and Skyfire — establishing the first dedicated identity-verification layer for agentic transactions. MoonPay announced MoonAgents Card, a virtual Mastercard debit card enabling AI agents to spend stablecoins directly from onchain wallets at any Mastercard-accepting merchant globally, while Mastercard and Rabobank completed the first AI agent-initiated payment in the Netherlands. The stakes — and the unresolved questions — are enormous.
...
This paper is intended as a living reference for engineers, product leaders, investors, and policymakers navigating this transition. It synthesizes publicly available data, protocol specifications, and analyst projections current through May 1, 2026.
...
- **Experian Agent Trust™:** Launched April 30, 2026, the first dedicated identity-verification framework for agentic commerce. Experian's "Know Your Agent" (KYA) framework provides three core components: (1) **Human-to-Agent Binding** — cryptographically linking an agent's actions to a verified consumer identity; (2) **Agent Trust Token** — a credential issued per-session that encodes consumer identity verification, agent authorization scope, and transaction risk signals; and (3) **Agent Registry** — a behavioral tracking system that records agent risk signals over time. Developed with Visa (Intelligent Commerce / Trusted Agent Protocol integration), Cloudflare (network-edge trust enforcement), and Skyfire (interoperability and agent identity). Experian's existing identity verification infrastructure helps clients avoid an estimated $15–19 billion in annual fraud losses; Agent Trust extends this to AI-initiated transactions [[91]](#ref91).
...
**Stripe Sessions 2026 (April 29).** At its annual conference, Stripe unveiled **288 new products and features** to more than 9,000 attendees, positioning itself as the default settlement layer for the agentic economy. CEO Patrick Collison declared that "in the not-too-distant future agents will account for most transactions online." Headline launches included: (1) **Link's wallet for agents** — built on Stripe's new **Issuing for agents**, consumers can grant AI agents programmatic access to Link and the ability to obtain one-time-use cards or Shared Payment Tokens backed by cards and bank accounts already in their wallet, without exposing raw payment credentials. Spend requests can be reviewed and approved via the web or Link's new iOS and Android apps. (2) **Google partnership** — merchants can now sell directly inside **Google AI Mode and the Gemini app** via the Agentic Commerce Suite, extending Stripe's agentic checkout to four of the largest AI platforms (OpenAI, Microsoft, Meta, Google). (3) **Checkout Studio** — build, analyze, and optimize high-converting checkouts in a few clicks. (4) **Stripe Treasury expansion** with instant, free money transfers between businesses on Stripe. Link now has **more than 250 million users globally**, making it the first mainstream consumer payment surface explicitly designed for the agent-to-agent era [[83]](#ref83) [[84]](#ref84).
...
**UCP Tech Council expansion.** On May 1, 2026, **Amazon, Meta, Microsoft, Salesforce, and Stripe officially joined the UCP Tech Council** — the technical governing body that steers the protocol's open standard. The move doubled the council's membership to 10 organizations, joining founding members Google, Shopify, Etsy, Target, and Wayfair. Amazon's participation is particularly notable given its historically walled-garden approach to commerce infrastructure; Meta's presence signals potential UCP integration into Instagram and Facebook Shops; and Stripe's membership aligns its Agentic Commerce Suite with UCP's checkout flows. Industry observers described the expansion as transforming UCP from a Google-Shopify initiative into something closer to the HTTP of commerce — a shared layer that any AI agent, from any platform, can use to transact with any merchant [[95]](#ref95).
...
**Google Business Agent.** Launched in January 2026, Business Agent lets shoppers chat with brands directly on Google Search — functioning as a virtual AI sales associate. Launch retailers include **Lowe's, Michaels, Poshmark, and Reebok**. Eligible U.S. retailers activate and customize their branded agent via Merchant Center. Google plans to expand Business Agent with retailer-trained models, customer insights, related product offers, and **agentic checkout** directly within the experience — positioning it as a merchant-owned commerce surface complementary to UCP [[77]](#ref77).
...
**Rufus agentic features.** Amazon's Rufus AI shopping assistant has reached **300 million users** and drove **$12 billion in incremental annualized sales** in 2025, according to Q4 2025 earnings materials — exceeding the $10 billion annualized pace disclosed after Q3 2025. In November 2025, Amazon added agentic capabilities including **auto-buy** and **price tracking** — customers set target prices and Rufus automatically purchases when the price drops. Rufus users are reportedly 60% more likely to complete a purchase [[33]](#ref33) [[78]](#ref78).
...
**Global expansion (April 29–May 1).** Visa announced the **global expansion of its Agentic Ready program to Asia Pacific and Latin America**, extending beyond the initial European launch. The APAC launch, formally announced May 1 in Singapore, enrolled **over 50 issuer partners across 10 markets** — a significantly larger rollout than the initial 21 European partners. Early APAC issuing partners include **DBS Bank, OCBC, UOB, HSBC, Standard Chartered, ANZ, ASB Bank, KakaoBank, Samsung Card, Hyundai Card, KB Kookmin Card, Shinhan Card, KASIKORNBANK (KBank), Maybank, CIMB, Cathay United Bank, CTBC Bank, Sumitomo Mitsui Card, Rakuten Card, Mitsubishi UFJ NICOS**, and dozens more across Australia, New Zealand, Singapore, Hong Kong, South Korea, Japan, Taiwan, Thailand, Vietnam, and Malaysia. T.R. Ramachandran (Visa Head of Products & Solutions, Asia Pacific) stated: "Asia Pacific is one of the most dynamic and digitally advanced payments regions in the world, making it an ideal environment to bring agentic commerce from concept to reality." The program validates how agent-initiated payments operate in production-grade environments while keeping consumers in control [[85]](#ref85) [[86]](#ref86) [[96]](#ref96).
...
**Mastercard Netherlands: first AI agent payment with Rabobank.** On April 30, 2026, Mastercard and **Rabobank** completed the **first AI agent-initiated payment in the Netherlands** — a live transaction in which an AI assistant booked a coffee tasting experience on Priceless.com using a Rabobank Mastercard credit card, without the consumer navigating to a webshop or clicking through a checkout flow. The transaction was orchestrated via **PayOS** and authenticated through **Mastercard Payment Passkeys**. The AI agent operated exclusively within permissions granted in advance by the consumer, with no access to card credentials. This brings Mastercard Agent Pay's live market count to **10+ countries**: Australia, New Zealand, South Korea, Singapore, Hong Kong, India, Latin America/Caribbean, Europe (Santander Spain, Rabobank Netherlands), and the U.S. [[92]](#ref92).
...
- **Clink** (San Francisco; launched April 30, 2026): Launched the world's first **fiat Agentic Payment Skill** — a production-ready capability that lets autonomous AI agents pay for products and services in real currencies on the user's existing credit card, under user-defined limits. Built on three core principles: (1) no new wallets — agents pay in fiat using cards and local payment methods users already trust; (2) payments from any authorized agent settle through existing payment processors; (3) per-use revenue layer sized to how agents consume. First Clink-enabled merchants include **ModelMax** and **PollyReach**. Public access is now open to AI builders and digital merchants worldwide [[87]](#ref87).
- **Alchemy AgentPay** (open beta launched April 28, 2026): A **unified integration for every agentic payment protocol** — x402, MPP, ACP, and others — allowing developers to accept payments from AI agents through a single connection regardless of which protocol the agent uses. Positioned as the aggregation layer that abstracts protocol fragmentation for merchants and service providers [[88]](#ref88).
- **Oobit Visa Agent Cards** (launched May 1, 2026; Tether-backed): Virtual **Visa cards funded directly from Tether's USDT treasury**, enabling AI agents to make online purchases in stablecoins without fiat on-ramps or currency conversions. Each AI agent receives a single card for clean identity and audit trails, with spend limits and merchant-category restrictions enforced at the transaction layer. Compatible with AI frameworks from OpenAI, Claude, AutoGen, and LangChain. Launched to a founding group of businesses with onboarding expanding through June 30, 2026. Oobit described the "next trillion users on the internet" as AI agents, echoing predictions from Coinbase CEO Brian Armstrong and Circle CEO Jeremy Allaire that billions of AI agents will transact on-chain within three to five years [[97]](#ref97).
- **OKX Agent Payments Protocol (APP)** (launched April 2026): An **open standard for the full agent commerce lifecycle**, going beyond single-payment execution to support quoting, negotiating, escrowing funds, metering usage, settling, and dispute resolution. Built on OKX's Onchain OS, APP is multi-chain (Solana, Ethereum) and designed for agents that manage ongoing commercial relationships rather than one-off transactions [[89]](#ref89).
- **MoonPay MoonAgents Card** (announced May 1, 2026; partnership with Monavate and Mastercard): A virtual Mastercard debit card purpose-built for AI agents to spend stablecoins directly from **self-custodial onchain wallets** at any Mastercard-accepting merchant globally. Unlike existing agent card products that require pre-loading a custodial balance or transferring funds offchain, MoonAgents Card authorizes spending directly from the onchain wallet at the moment of transaction, with crypto-to-fiat conversion at the point of purchase via Monavate's regulated card infrastructure. Accessible programmatically through **MoonPay CLI** and **MoonPay Agents workflows** — MoonPay CLI has processed over **4 million tool calls**, with the second million processed in just seven days. MoonPay is also co-founding the **Open Wallet Standard** (backed by 15+ organizations) for cross-chain value holding by AI agents. Live in the UK and Latin America; U.S. and EU availability forthcoming [[93]](#ref93).
- **PhotonPay** (Hong Kong-based; dual-rail recurring billing launched May 1, 2026): Unveiled a **dual-rail recurring billing system** enabling businesses to manage subscription and usage-based payments across both fiat and stablecoin rails through a single integration. Targets AI-native enterprises where 20–40% of subscription churn is caused by passive payment failures (cross-border card declines, currency conversion errors) rather than product dissatisfaction. The system introduces programmable recurring billing on stablecoin rails — previously, on-chain payments were limited to discrete manual transactions. Also launched a **Multi-Currency Global Collections Wallet** bridging fiat currencies and compliant stablecoins as a foundational execution layer for the agentic economy [[94]](#ref94).
- **Mirakl Nexus + J.P. Morgan Payments** (strategic partnership announced March 10, 2026): Mirakl's **Nexus** platform serves as the agentic commerce layer — optimizing product catalogs for AI discovery and enabling the full shopping journey from discovery through checkout and aftersales across LLM channels (Gemini, Copilot, Perplexity). J.P. Morgan Payments provides enterprise payment infrastructure: secure payment processing, tokenization enabling AI agents to transact safely, and fraud protection at enterprise scale. The partnership is in **closed beta** with select retailers and merchants, with broader availability planned for 2026. Nexus + J.P. Morgan represents the first major enterprise marketplace platform to build dedicated agentic commerce infrastructure with a top-tier payments processor [[79]](#ref79).
...
|---|---|---|---|---|---|---|
| **Stripe / Tempo** | Machine Payments Protocol (MPP) + Agentic Commerce Suite + Link Agent Wallet | Open spec; session-based micropayments; fiat + stablecoin via SPTs; PaymentIntents API; Visa + Lightning + Mastercard Agent Pay + BNPL (Affirm, Klarna); Agentic Commerce Suite; Meta/Facebook checkout; **Link's wallet for agents** (Issuing for agents, one-time-use cards, iOS/Android approval); **Google AI Mode + Gemini** partnership; Checkout Studio; 250M+ Link users | Processing fees (~2.9% + $0.30); $1.9T volume (2025); $159B valuation; 288 launches at Sessions 2026 | GA (March 2026; Sessions expansion April 2026) | Agent-to-service payments, micropayments, compute billing, AI metering, social commerce, consumer agent wallets | Open |
...
| **Google** | UCP + A2A + AP2 + Business Agent | Open standards; buy button in Search/Gemini; Cart, Catalog, Identity Linking (March 2026); A2A for agent coordination; PayPal UCP integration; AP2 consortium (60+ companies, Revolut Pay A2A integration); **Business Agent** (branded AI sales agents on Search); **UCP Tech Council expanded to 10 members** (Amazon, Meta, Microsoft, Salesforce, Stripe joined May 1) | Ads/shopping commissions (likely) | GA (Jan 2026; expanded March 2026; Tech Council doubled May 2026) | Product discovery, multi-item checkout, post-purchase, agent-to-agent, account-to-account payments, merchant-branded search agents | Open |
...
| **Amazon** | Buy for Me / Shop Direct / Rufus | Nova + Claude on Bedrock; Nova Act browser automation; feed-based merchant opt-in (Feedonomics, Salsify, CEDCommerce) | Marketplace commissions; $50B OpenAI partnership; Rufus: $12B incremental annualized sales (2025) | GA (expanding; 100M+ products, 400K+ merchants; 300M Rufus users) | Cross-merchant shopping, auto-buy, price tracking | Closed |
...
| **Visa** | Trusted Agent Protocol + Agentic Ready + AI Dispute Suite | Cryptographic agent verification; card-network settlement; Agentic Ready testing (21 EU + **50+ APAC** partners across 10 markets — DBS, OCBC, UOB, HSBC, Standard Chartered, KakaoBank, Samsung Card, ANZ, Maybank, CIMB, etc.); **global expansion to APAC + LatAm** (April–May 2026); **AI Dispute Resolution Suite** (6 tools, 106M disputes/yr); **Ramp agentic corporate bill pay** (50K+ clients) | Network interchange fees | GA (Agentic Ready March 2026; APAC 50+ partners May 2026; Dispute Suite April 2026) | Agent identity verification, fraud prevention, bank testing, corporate bill pay, dispute resolution | Consortium |
| **Mastercard** | Agent Pay + Verifiable Intent | Dynamic tokenization; cryptographic mandate binding (with Google); purchase-intent metadata; live in AU/NZ/LATAM/KR/SG/HK/IN/EU/US/**NL** (Rabobank April 30); **Hong Kong first live transaction** (HSBC, DBS, Citi, Hang Seng, Standard Chartered, Mox — April 2026); **Netherlands first live transaction** (Rabobank, PayOS, Priceless.com — April 30); AI Centre of Excellence (Singapore) | Network interchange fees | GA (Agent Pay live in 10+ markets; expanding) | Tokenized agent payments, recurring mandates, merchant verification | Consortium (Verifiable Intent open-source) |
...
| **Alibaba** | Autonomous Digital Workers (Taobao/Tmall) + Qwen3.6-Plus + Alipay AI Payments | Built on Business Advisor platform; Qwen3.6-Plus (1M-token context, agentic coding, multimodal); Alibaba Token Hub; Wukong enterprise platform; Dianxiaomi AI (300M customers, 30% conversion lift); 5M merchants using AI tools, 100B yuan savings; **Alipay AI payment product** (agent-to-merchant payments for businesses/OPCs) | Marketplace commissions; advertising; AI model licensing; payment processing | GA (April 2026; millions of merchants; Qwen3.6-Plus April 2; Alipay AI Payments April 28) | Merchant-side agents: customer service, dynamic pricing, voucher distribution, agentic coding, enterprise workflows, agent payment acceptance | Mixed (Qwen3.6 series partially open-source; Qwen3.6-Plus closed) |
...
| **Clink** | Fiat Agentic Payment Skill | Agents pay in fiat on existing credit cards; user-defined spending limits; per-use revenue layer; no new wallets required | Processing fees | GA (April 2026) | Merchant payment acceptance from AI agents, micropayments, per-use billing | — |
| **Alchemy** | AgentPay | Unified integration for all agentic payment protocols (x402, MPP, ACP); single connection for multi-protocol support | Transaction/platform fees | Open Beta (April 2026) | Protocol-agnostic agent payment acceptance, compute procurement | — |
| **Oobit** | Visa Agent Cards (Tether-backed) | Virtual Visa cards funded from Tether USDT treasury; no fiat on-ramp; per-agent card issuance; spend limits + MCC restrictions; OpenAI/Claude/AutoGen/LangChain compatible | Card issuance/processing fees | Launched (May 1, 2026; expanding through June 30) | Agent stablecoin spending, SaaS renewals, ad budget top-ups, cloud infrastructure procurement | Closed |
| **OKX** | Agent Payments Protocol (APP) | Open standard for full commerce lifecycle; quoting, negotiating, escrowing, metering, settling; multi-chain (Solana, Ethereum) | Transaction fees | Launched (April 2026) | Agent-to-agent commerce, escrow, usage metering, dispute resolution | Open |
...
| Juniper Research | $1.5 trillion globally; trust identified as #1 adoption barrier | By 2030 |
...
| BCG | AI agent-led shopping could represent 25%+ of e-commerce spending | Next several years |
| eMarketer | AI platform-driven e-commerce sales surpass $144 billion (8.8% of retail e-commerce) | By 2029 |
...
Morgan Stanley's AlphaWise survey indicates LLM adoption is approaching 50% in the U.S., with AI agents capturing 10–20% of e-commerce spend [[12]](#ref12). Bain & Co. projects a wider range ($300–500B) based on higher penetration assumptions (15–25% of overall U.S. e-commerce by 2030) [[58]](#ref58). **Juniper Research** (April 2026) projects **$1.5 trillion** in global agentic commerce spend by 2030, growing from pilot deployments in 2025–2026 — but identifies **trust as the number one barrier** to deployment, tempering the more aggressive McKinsey estimates [[80]](#ref80). BCG (September 2025) projects AI agent-led shopping could represent **over 25% of e-commerce spending** within the next several years, while eMarketer forecasts AI platform-driven e-commerce sales will surpass **$144 billion by 2029** (8.8% of total retail e-commerce) [[81]](#ref81). JPMorgan's payments division has identified agentic commerce as a top-three strategic priority, noting that "agent-initiated transactions introduce a new category altogether" — payments executed on behalf of consumers, sometimes with human involvement but increasingly fully autonomous [[13]](#ref13).
...
**Near-term adoption calibration.** At NRF 2026, **75% of retailer attendees** said they were implementing or planning agentic commerce. However, Stripe's co-founders offered a sobering counterpoint at Shoptalk Spring 2026: they expect **95% of AI-driven sales this year will still complete on merchant websites**, not inside AI interfaces — characterizing parts of the agentic commerce narrative as "overhyped too early" [[82]](#ref82). This gap between infrastructure investment and actual consumer adoption suggests a multi-year transition where the merchant checkout experience persists as the dominant conversion surface even as AI-driven product discovery scales rapidly.
...
- **Human-in-the-loop checkpoints:** Most current implementations (ChatGPT Instant Checkout, Amazon Buy for Me, Copilot Checkout) still require human confirmation at the payment step. Fully autonomous purchasing remains limited to lower-value B2B transactions. Criteo's 2026 Commerce and AI Trend Report found that **55% of shoppers who use AI are extra-cautious about sharing payment information** with AI tools — underscoring the consumer trust deficit that Juniper Research identified as the primary barrier to agentic commerce deployment [[81]](#ref81).
...
Six or more major protocols (MPP, ACP, UCP, x402, ATXP, APP) are live simultaneously, with aggregation layers like Alchemy's AgentPay emerging to abstract the fragmentation. Will the market converge on one standard, or will a multi-protocol world persist? UCP's explicit compatibility with A2A, ACP, and MCP suggests a layered rather than winner-take-all outcome, while ATXP's and OKX APP's entries signal continued startup innovation in the payment layer. The emergence of protocol aggregators suggests the market may settle on a multi-protocol reality with middleware abstraction rather than a single winner. The May 2026 expansion of the UCP Tech Council to include Amazon, Meta, Microsoft, Salesforce, and Stripe — alongside founding members Google, Shopify, Etsy, Target, and Wayfair — significantly shifts the convergence calculus: UCP now has the weight of the entire digital commerce stack behind it, though whether these historically competitive companies can co-govern a shared commerce layer remains an open question. Convergence is far from certain.
...
<a id="ref77"></a>
**[77]** Google. "Introducing Business Agent on Google Search." January 2026. — See also: Think with Google. "Business Agent lets shoppers chat with brands on Search." https://www.thinkwithgoogle.com/marketing-strategies/automation/google-business-agent/

<a id="ref78"></a>
**[78]** Amazon. "Q4 2025 Earnings — Rufus AI Shopping Assistant Metrics." February 2026. — See also: PYMNTS. "Amazon's Rufus AI Assistant Reaches 300M Users, Drives $12B in Incremental Annualized Sales." February 2026.

<a id="ref79"></a>
**[79]** Mirakl. "Mirakl and J.P. Morgan Payments Partner on Agentic Commerce for Enterprise Marketplaces." March 10, 2026. https://www.mirakl.com/news/mirakl-jpmorgan-payments-agentic-commerce — See also: PYMNTS. "Mirakl, J.P. Morgan Partner to Bring Agentic Commerce to Enterprise Marketplaces." March 2026.

<a id="ref80"></a>
**[80]** Juniper Research. "Agentic Commerce: Market Forecasts, Key Trends & Competitive Landscape 2026–2030." April 2026. — Key finding: $1.5 trillion global agentic commerce spend by 2030; trust identified as #1 barrier to deployment.

<a id="ref81"></a>
**[81]** BCG. "AI Agent-Led Shopping Could Represent Over 25% of E-Commerce Spending." September 2025. — See also: eMarketer. "AI Platform-Driven E-Commerce Sales Forecast." 2026. — See also: Criteo. "2026 Commerce and AI Trend Report." 2026 (55% of AI-using shoppers extra-cautious about sharing payment info).

<a id="ref82"></a>
**[82]** PYMNTS. "Stripe Co-Founders at Shoptalk: 95% of AI-Driven Sales Will Still Complete on Merchant Websites This Year." April 2026. — See also: NRF 2026 attendee survey (75% of retailers implementing or planning agentic commerce).

<a id="ref83"></a>
**[83]** Stripe. "Giving agents the ability to pay." April 29, 2026. https://stripe.com/blog/giving-agents-the-ability-to-pay

<a id="ref84"></a>
**[84]** Stripe. "Stripe builds out the economic infrastructure for AI with 288 launches." April 29, 2026. https://stripe.com/newsroom/news/sessions-2026 — See also: Unite.AI. "Stripe Hands AI Agents a Wallet, Ushering In Agentic Purchasing." April 30, 2026. https://www.unite.ai/stripe-hands-ai-agents-a-wallet-ushering-in-agentic-purchasing/ — See also: Payments Dive. "Stripe, Google partner on agentic commerce." April 29, 2026. https://www.paymentsdive.com/news/stripe-google-partner-on-agentic-commerce/818915/

<a id="ref85"></a>
**[85]** Visa. "Visa Announces Global Expansion of Agentic Ready Program." April 29, 2026. https://www.nasdaq.com/press-release/visa-announces-global-expansion-agentic-ready-program-2026-04-29 — See also: PYMNTS. "Visa Expands Agentic Ready Program to LatAm and Asia." April 29, 2026. https://www.pymnts.com/visa/2026/visa-expands-agentic-ready-program-to-latam-and-asia/

<a id="ref86"></a>
**[86]** eCommerceNews Australia. "Visa adds Australian banks to agentic payments trial." April 29, 2026. https://ecommercenews.com.au/story/visa-adds-australian-banks-to-agentic-payments-trial — See also: FinnoEx. "Visa Launches Agentic Ready AI Payment Program in Singapore." April 30, 2026. https://finnoex.com/visa-launches-agentic-ready-ai-payment-program-in-singapore/

<a id="ref87"></a>
**[87]** GlobeNewsWire. "Clink Launches the World's First Fiat Agentic Payment Skill, Letting Any Merchant Get Paid by AI Agents." April 30, 2026. https://www.globenewswire.com/news-release/2026/04/30/3284590/0/en/Clink-Launches-the-World-s-First-Fiat-Agentic-Payment-Skill-Letting-Any-Merchant-Get-Paid-by-AI-Agents.html

<a id="ref88"></a>
**[88]** Alchemy. "Introducing AgentPay: one integration for every agentic payment protocol." April 28, 2026. https://www.alchemy.com/blog/agentpay-openbeta

<a id="ref89"></a>
**[89]** OKX. "Agents can now do real business, not just make payments — Introducing the Agent Payments Protocol." April 2026. https://www.okx.com/en-gb/learn/agent-payments-protocol

<a id="ref90"></a>
**[90]** BusinessWire. "Alipay Launches AI Payment Processing Product to Help Businesses and OPCs Thrive in the Agentic Economy." April 28, 2026. https://www.businesswire.com/news/home/20260428546438/en/Alipay-Launches-AI-Payment-Processing-Product-to-Help-Businesses-and-OPCs-Thrive-in-the-Agentic-Economy — See also: The Paypers. "Alipay launches AI payment product for agent commerce." April 28, 2026. https://thepaypers.com/payments/news/alipay-launches-ai-payment-processing-product-for-agentic-commerce

<a id="ref91"></a>
**[91]** Experian. "Experian Agent Trust™: Powering The Next Wave Of AI-Driven Commerce." April 30, 2026. https://www.experian.com/blogs/news/2026/04/30/experian-agent-trust/ — See also: BusinessWire. "Experian Announces Agent Trust to Power Trusted AI Driven Commerce." April 30, 2026. https://www.businesswire.com/news/home/20260430719198/en/Experian-Announces-Agent-Trust-to-Power-Trusted-AI-Driven-Commerce

<a id="ref92"></a>
**[92]** PPC Land. "Mastercard and Rabobank complete first AI agent payment in the Netherlands." May 1, 2026. https://ppc.land/mastercard-and-rabobank-complete-first-ai-agent-payment-in-the-netherlands/ — See also: Finance Innovation NL. "Eerste Nederlandse AI-agent betaling ooit." May 1, 2026. https://financeinnovation.nl/mastercard-rabobank-ai-agent-betaling/

<a id="ref93"></a>
**[93]** PRNewswire. "MoonPay Announces 'MoonAgents Card' Enabling AI Agents to Spend Stablecoins Anywhere Mastercard Is Accepted." May 1, 2026. https://www.prnewswire.com/news-releases/moonpay-announces-moonagents-card-enabling-ai-agents-to-spend-stablecoins-anywhere-mastercard-is-accepted-302760128.html — See also: Stablecoin Insider. "MoonPay Launches MoonAgents Card: A Virtual Mastercard Debit Card for Stablecoins and AI Agents." May 1, 2026. https://stablecoininsider.org/moonpay-launches-moonagents-card-a-virtual-mastercard-debit-card-for-stablecoins-and-ai-agents/

<a id="ref94"></a>
**[94]** PaymentsJournal. "Fueling Agentic Commerce with Dual-Rail Recurring Billing." May 1, 2026. https://www.paymentsjournal.com/fueling-agentic-commerce-with-dual-rail-recurring-billing/ — See also: The Paypers. "PhotonPay launches dual-rail recurring billing for AI subscriptions." May 1, 2026. https://thepaypers.com/payments/news/photonpay-launches-dual-rail-recurring-billing-for-ai-subscriptions

<a id="ref95"></a>
**[95]** Endcap Brief. "Amazon, Meta, and Microsoft Just Signed On to the Protocol That Could Define How AI Shops for You." May 2, 2026. https://endcap.news/technology/ucp-tech-council-amazon-meta-microsoft-agentic-commerce-2026 — See also: Newsfile. "Amazon, Meta, Microsoft, Salesforce, and Stripe Join the Universal Commerce Protocol Tech Council." May 1, 2026. https://www.newsfilecorp.com/release/294133/Amazon-Meta-Microsoft-Salesforce-and-Stripe-Join-the-Universal-Commerce-Protocol-Tech-Council

<a id="ref96"></a>
**[96]** Antara News / PRNewswire. "Visa Launches Agentic Ready Program in Asia Pacific with Over 50 Partners — Advancing Agentic Commerce." May 1, 2026. https://en.antaranews.com/amp/news/414344/visa-launches-agentic-ready-program-in-asia-pacific-with-over-50-partners-advancing-agentic-commerce
...
| Date | Change |
|---|---|
| 2026-05-03 | **v10 — May 3 update.** Abstract updated with UCP Tech Council expansion (Amazon, Meta, Microsoft, Salesforce, Stripe joined May 1 — doubling membership to 10) and Visa APAC detail (50+ issuer partners across 10 markets). §4.3: Added UCP Tech Council expansion — Amazon, Meta, Microsoft, Salesforce, and Stripe officially joined governing body (May 1), transforming UCP from Google-Shopify initiative to multi-stakeholder industry standard. §4.8: Updated Visa Agentic Ready APAC section with 50+ named issuer partners across 10 markets (DBS, OCBC, UOB, HSBC, Standard Chartered, KakaoBank, Samsung Card, ANZ, Maybank, CIMB, Cathay United, CTBC, Sumitomo Mitsui, Rakuten, etc.). §4.10: Added Oobit Visa Agent Cards (Tether-backed virtual Visa for AI agents to spend USDT; per-agent card issuance with spend limits; launched May 1 to founding businesses). §5: Updated comparison table — revised Google row (Tech Council expansion), Visa row (50+ APAC partners, 10 markets); added Oobit row. §11.7: Updated protocol convergence analysis with UCP Tech Council implications. Added 3 new references ([95]–[97]). |
| 2026-05-02 | **v9 — May 2 update.** Abstract updated with Experian Agent Trust, MoonPay MoonAgents Card, and Mastercard/Rabobank Netherlands first transaction. §3.2: Added Experian Agent Trust™ — first dedicated identity-verification framework for agentic commerce with "Know Your Agent" (KYA) framework including Human-to-Agent Binding, Agent Trust Token, and Agent Registry; developed with Visa, Cloudflare, and Skyfire. §4.8: Added Mastercard + Rabobank Netherlands first live AI agent payment (April 30 — coffee tasting on Priceless.com via PayOS and Payment Passkeys); updated Agent Pay live market count to 10+. §4.10: Added MoonPay MoonAgents Card (virtual Mastercard for AI agents to spend stablecoins from self-custodial onchain wallets; 4M+ MoonPay CLI tool calls; Open Wallet Standard with 15+ orgs; live UK/LATAM); added PhotonPay dual-rail recurring billing (fiat + stablecoin subscription management for AI-native enterprises; addresses 20–40% passive churn from payment failures). §5: Updated comparison table — revised Mastercard row (Netherlands/Rabobank, 10+ markets); added Experian, MoonPay, PhotonPay rows. Added 4 new references ([91]–[94]). |
| 2026-05-01 | **v8 — May 1 update.** Abstract updated with Stripe Sessions 2026 highlights (288 launches, Link's wallet for agents, Google partnership) and new entrants (Alchemy AgentPay, OKX APP, Clink). §4.1: Added Stripe Sessions 2026 — Link's wallet for agents (Issuing for agents, one-time-use cards/SPTs, iOS/Android approval apps, 250M+ Link users), Google partnership (sell inside AI Mode and Gemini app), Checkout Studio, Treasury expansion. §4.8: Added Visa Agentic Ready global expansion to Asia Pacific (Australia, Singapore) and Latin America (April 29). §4.10: Added Clink (first fiat Agentic Payment Skill), Alchemy AgentPay (unified multi-protocol integration, open beta), OKX Agent Payments Protocol/APP (full commerce lifecycle standard with escrow, metering, dispute resolution). §4.11: Added Alipay AI payment processing product for businesses/OPCs in China (April 28). §5: Updated comparison table — revised Stripe row (Sessions 2026 launches, Link wallet, Google), Visa row (global APAC/LatAm expansion), Alibaba row (Alipay AI Payments); added Clink, Alchemy, OKX rows. §11.7: Updated protocol count to 6+; noted emergence of protocol aggregators (AgentPay) suggesting multi-protocol future with middleware abstraction. Added 8 new references ([83]–[90]). |
| 2026-04-04 | **v7 — April 4 update.** Abstract updated with Juniper Research $1.5T projection (trust as #1 barrier), Amazon Rufus 300M users/$12B incremental sales, and Mirakl + J.P. Morgan Payments partnership. §4.3: Added Google Business Agent (branded AI sales agents on Search — Lowe's, Michaels, Poshmark, Reebok; agentic checkout planned). §4.5: Updated Rufus to 300M users and $12B incremental annualized sales (Q4 2025 earnings). §4.10: Added Mirakl Nexus + J.P. Morgan Payments strategic partnership (closed beta for enterprise marketplace agentic commerce). §5: Updated comparison table — revised Google row (Business Agent), Amazon row (300M Rufus users, $12B sales). §7: Added Juniper Research ($1.5T by 2030, trust barrier), BCG (25%+ of e-commerce), eMarketer ($144B by 2029) projections; added Stripe co-founders' Shoptalk counterpoint (95% of AI sales still on merchant sites); added NRF 2026 retailer adoption survey (75% implementing/planning). §8.3: Added Criteo finding (55% of AI shoppers cautious about payment info sharing). Added 6 new references ([77]–[82]). |
