...
**Instant Checkout pivot.** By March 2026, OpenAI's initial Instant Checkout approach — completing purchases entirely within ChatGPT — had been sidelined. Only ~30 Shopify merchants were live, onboarding proved arduous, and high-intent browsers showed low conversion rates. OpenAI pivoted to a model where retailers build dedicated in-ChatGPT app experiences (Walmart launched account linking with loyalty programs; Shopify enabled merchant storefronts) while handling checkout on their own platforms. Target, Sephora, Nordstrom, Lowe's, Best Buy, The Home Depot, and Wayfair have integrated ACP for product discovery [[24]](#ref24).
...
|---|---|---|---|---|---|---|
| **Stripe / Tempo** | Machine Payments Protocol (MPP) | Open spec; session-based micropayments; fiat + stablecoin via SPTs; PaymentIntents API; Visa + Lightning support | Processing fees (~2.9% + $0.30); $1.9T volume (2025); $159B valuation | GA (March 2026) | Agent-to-service payments, micropayments, compute billing, AI metering | Open |
| **OpenAI** | Instant Checkout / ACP | Integrated in ChatGPT; Operator for web automation; GPT-5.4 native computer use (75% OSWorld) | 4% merchant tx fee + processing; $280B revenue projected by 2030 | GA (Sept 2025); pivoting to retailer apps (March 2026) | Consumer shopping, product discovery, booking, SaaS replacement | Open (ACP Apache 2.0) |
| **Google** | UCP + A2A | Open standards; buy button in Search/Gemini; Cart, Catalog, Identity Linking (March 2026); A2A for agent coordination | Ads/shopping commissions (likely) | GA (Jan 2026; expanded March 2026) | Product discovery, multi-item checkout, post-purchase, agent-to-agent | Open |
...
| **Anthropic** | MCP / Computer Use / Claude | Open protocol (MCP v3: OAuth, elicitation, structured output); Mac computer use; buffa/connect-rust (33% faster decode) | API usage fees; enterprise licensing | GA (MCP); Research Preview (Computer Use Mac) | Tool integration, web automation, enterprise agents, infrastructure | Open (MCP) |
...
| **Mastercard** | Agent Pay + Verifiable Intent | Dynamic tokenization; cryptographic mandate binding (with Google); purchase-intent metadata; live in AU/NZ/LATAM/KR | Network interchange fees | GA (Agent Pay live in multiple markets) | Tokenized agent payments, recurring mandates, merchant verification | Consortium (Verifiable Intent open-source) |
